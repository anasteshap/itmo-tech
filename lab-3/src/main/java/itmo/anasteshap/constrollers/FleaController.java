...
public class FleaController {
    ...
    public ResponseEntity<?> createFlea(@Validated @RequestBody CreateFleaRequest request) {
        return new ResponseEntity<>(fleaService.save(request), HttpStatus.OK);
    }

    @GetMapping("/{id}")
    @Operation(summary = "Получить блоху по ID")
    public ResponseEntity<?> getFleaById(@Validated @PathVariable Long id) {
        return new ResponseEntity<>(fleaService.getById(id), HttpStatus.OK);
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "Удалить блоху по ID")
    public ResponseEntity<?> deleteFleaById(@Validated @PathVariable Long id) {
    ...
    public ResponseEntity<?> updateFlea(@Validated @RequestBody UpdateFleaRequest catRequest) {
        return new ResponseEntity<>(fleaService.update(catRequest), HttpStatus.OK);
    }

    @GetMapping
    @Operation(summary = "Получить информацию о всех блохах")
    public ResponseEntity<?> getFleas(@RequestParam(required = false, defaultValue = "1") Integer page,
    ...
    public ResponseEntity<?> getAllByOwnerId(@Validated @PathVariable("cat_id") Long id) {
